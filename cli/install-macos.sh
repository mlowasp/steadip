...
main() {
  ...
  case "$os" in
    ...
    *) fail "This installer is for macOS only. Detected: $os" ;;
  esac
  ...
  # Remove quarantine attribute when installed from browser-downloaded locations or copied around.
  ...
  if "$BIN_PATH" --help >/dev/null 2>&1; then
    ...
    warn "If macOS blocks it, run: xattr -d com.apple.quarantine \"$BIN_PATH\""
  fi
